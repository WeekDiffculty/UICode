...
- (void)setAppModel:(QYAppModel *)appModel {
    ...
    [_imageView setImage:[UIImage imageNamed: appModel.icon]];
    [_lblAppName setText:appModel.name];
}
...
- (IBAction)tapAction {
    ...
    if (self.blkDidTapDownloadButton) {
        // 调用代码块
        self.blkDidTapDownloadButton(self.appModel);
    }
}
